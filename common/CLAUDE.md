...
* Whenever any substantial code change is made (including but not limited to anything that would cause you to modify the
  `README.md` file) you should update the `.claude/CLAUDE.md` file for the project. This file is for you to regain context
  quickly in the future without having to consume the entirety of the project's code.
...
* You can assume that the environment you're running in has `mise` installed to handle tool management. If you're missing a tool
  that can be installed with `mise` you can either create or use the existing `mise.toml` (or `.mise.toml`) in the root of the
  project and then run `mise install` to install the tool. If you want to automate tasks you can also use `mise.toml` (or
  `.mise.toml`) to do that (e.g. build tasks).
